...
object CKMImageNetTrain extends Serializable with Logging {
  ...
  def run(sc: SparkContext, conf: CKMImageNetTrainConf) {
    var data = loadTrain(sc, conf.dataset, conf.featureDir, conf.labelDir)
    println("RUNNING CKMImageNetTrain")
    val feature_id = conf.seed + "_" + conf.dataset + "_" +  conf.expid  + "_" + conf.layers + "_" + conf.patch_sizes.mkString("-") + "_" + conf.bandwidth.mkString("-") + "_" + conf.pool.mkString("-") + "_" + conf.poolStride.mkString("-") + "_" + conf.filters.mkString("-")
    ...
    val patchExtractor = new Windower(1, conf.patch_sizes(0))
      ...
      xs.map(mi => breeze.linalg.csvwrite(new File(s"${conf.modelDir}/${feature_id}.model.${mi._2}"), mi._1, separator = ','))
      model.bOpt.map(b => breeze.linalg.csvwrite(new File(s"${conf.modelDir}/${feature_id}.intercept"),b.toDenseMatrix, separator = ','))
      breeze.linalg.csvwrite(new File(s"${conf.modelDir}/${feature_id}.whitener.matrix"),whitener.whitener, separator = ',')
      breeze.linalg.csvwrite(new File(s"${conf.modelDir}/${feature_id}.whitener.means"),whitener.means.toDenseMatrix, separator = ',')
  ...
  def main(args: Array[String]) = {
    ...
    if (args.size < 1) {
      ...
      println(configtext)
      val yaml = new Yaml(new Constructor(classOf[CKMImageNetTrainConf]))
      val appConfig = yaml.load(configtext).asInstanceOf[CKMImageNetTrainConf]
